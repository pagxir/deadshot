...
static nat_conntrack_t * lookup_session(struct sockaddr_in6 *from, uint16_t sport)
{
    nat_conntrack_t *item;
    static uint32_t ZEROS[4] = {};

    int hash_idx0 = get_connection_match_hash(&from->sin6_addr, ZEROS, sport, from->sin6_port);
...
static void do_udp_exchange_back(void *upp)
{
	...
	count = recvfrom(up->sockfd, buf, sizeof(buf), 0, inaddr, &in_len);
	tx_aincb_update(&up->file, count);

	if (count <= 0) {
	    LOG_DEBUG("back recvfrom len %d, %d, strerr %s", count, errno, strerror(errno));
	    break;
	}

	struct sockaddr *inp = (struct sockaddr *)&up->source;
	count = sendto(up->sockfd, buf, count, MSG_DONTWAIT, inp, sizeof(up->source));
	if (count == -1) {
...
static nat_conntrack_t * newconn_session(struct sockaddr_in6 *from, int mainfd, int sport)
{
    int sockfd;
    int rcvbufsiz = 4096;
	...
	static uint32_t ZEROS[4] = {};
        conn->hash_idx = get_connection_match_hash(&from->sin6_addr, ZEROS, sport, from->sin6_port);
    }
...
static void do_udp_exchange_recv(void *upp)
{
	...
	session = session? session: newconn_session(&in6addr, up->sockfd, up->port);
	if (session != NULL) {
	...
	count = sendto(up->sockfd, buf, count, MSG_DONTWAIT, inp, sizeof(session->target));
	if (count == -1) {
...
static void * udp_exchange_create(int port)
{
	int sockfd;
	int error = -1;
	struct sockaddr_in6 in6addr;
