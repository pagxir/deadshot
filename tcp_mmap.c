...
void *child_thread(void *arg)
{
        ...
        assert(rc != -1);
        assert(nullfd != -1);
        int readfd = files[0], writefd = files[1];
        rc = fcntl(writefd, F_SETPIPE_SZ, chunk_size);
        assert(rc != -1);
        ...
        while (1) {
                ...
                if (zflg) {
                        ...
                        if (zc.length) {
                                ...
                                if ((last_check ^ time(NULL)) >> 2) {
                                    fprintf(stderr, "zc.length: %d zc.recv_skip_hint: %d\n", zc.length, zc.recv_skip_hint);
                                    time(&last_check);
                                }
                        ...
                        if (zc.recv_skip_hint) {
                                ...
                                lu = splice(fd, NULL, writefd, NULL,
                                                min(chunk_size, FILE_SZ - total), SPLICE_F_MOVE| SPLICE_F_NONBLOCK);
                                if (lu > 0) {
                                        splice(readfd, NULL, nullfd, NULL, lu, SPLICE_F_MOVE| SPLICE_F_NONBLOCK);
                                        total += lu;
                                }
                                if (lu == 0)
                                        goto end;
