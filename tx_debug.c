...
static FILE *log_get(int fd, const char *tag, void *cookie)
{
    time_t now;
    size_t count;
	struct timeval tv0;
...
static int log_put(FILE *log, void *cookie)
{
	struct log_cookie *logcb = (struct log_cookie *)cookie;
...
int log_tag_putlog(const char *tag, const char *fmt, ...)
{
	...
	va_list ap;
	va_start(ap, fmt);
    struct log_cookie _log_cookie = {};

	if (log_logable(tag)) {
#if 0
		FILE *logfp = log_get(STDERR_FILENO, tag, &_log_cookie);
		n = vfprintf(logfp, _logfmt(fmt, tag), ap);
		log_put(logfp, &_log_cookie);
#else
		char buf[4096];
		n = vsnprintf(buf, sizeof(buf), _logfmt(fmt, tag), ap);
