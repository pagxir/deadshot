...
static int install_maping(int port, int dport, int sockfd)
{
...
static nat_conntrack_t * lookup_session(struct sockaddr_in6 *from)
{
...
static void do_udp_exchange_back(void *upp)
{
    ...
    while (tx_readable(&up->file)) {
		...
		if (memcmp(v4mapped, &in6addr.sin6_addr, 13) == 0) {
			...
			up->mainfd = pick_from_port(dest.sin6_port);
		}
...
static nat_conntrack_t * newconn_session(struct sockaddr_in6 *from)
{
    int sockfd;
    int rcvbufsiz = 4096;
...
static void do_udp_exchange_recv(void *upp)
{
    ...
    while (tx_readable(&up->file)) {
        ...
        session = lookup_session(&in6addr);
        session = session? session: newconn_session(&in6addr);
        if (session == NULL) {
...
static void * udp_exchange_create(int port, int dport)
{
	...
	install_maping(port, dport, sockfd);
